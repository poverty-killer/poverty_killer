...
class ToxicityAlert:
    ...
    vpin_proxy: float              # Notional imbalance proxy (0-1)
...
class ToxicityEngine:
    ...
    TOXICITY COMPONENTS:
    - VPIN proxy: notional imbalance / directional pressure (NOTIONAL)
    ...
    SERIALIZATION_VERSION = 4  # Version 4: per-channel monotonic timestamps + full history continuity + buy/sell bucket splits

    def __init__(
        self,
        symbol: str,
        toxicity_threshold: float = 0.75,
        volume_bucket_units: float = 1_000_000.0,  # Deprecated; interpreted as notional if notional_bucket_value is None.
        ...
        notional_bucket_value: Optional[float] = None
    ):
        """
        Initialize toxicity engine.

        VPIN bucket volume is USD notional.


        Args:
            symbol: Trading symbol
            toxicity_threshold: Threshold for TOXIC regime (0-1)
            notional_bucket_value: Notional value per VPIN bucket (preferred; NOTIONAL-ONLY, in USD)
            volume_bucket_units: Deprecated; if provided as a non-default value and notional_bucket_value is None, it is treated as the notional bucket size (compat, in USD). Otherwise, per-symbol defaults are applied (BTC/XBT=100k, ETH=50k, SOL=20k; fallback=50k USD).
        ...
        # VPIN bucket configuration - NOTIONAL ONLY
        self.volume_bucket_units = volume_bucket_units  # deprecated: retained for BC
        if notional_bucket_value is not None:
            self.notional_bucket_value = float(notional_bucket_value)
        else:
            # Backward-compat: honor non-default volume_bucket_units if provided
            if float(volume_bucket_units) != 1_000_000.0:
                self.notional_bucket_value = float(volume_bucket_units)
                logger.info(f"ToxicityEngine: using legacy volume_bucket_units as notional bucket (compat) = {self.notional_bucket_value:,.0f} USD for {symbol}")
            else:
                # Per-symbol defaults; fallback to 50k USD
                self.notional_bucket_value = float(self._resolve_default_notional_bucket(symbol))
                logger.info(f"ToxicityEngine: using default notional VPIN bucket of {self.notional_bucket_value:,.0f} USD for {symbol}")

        # Sync deprecated field to resolved notional bucket for internal use
        self.volume_bucket_units = self.notional_bucket_value
        ...
        # Current bucket state (in NOTIONAL dollars for VPIN)
        ...
        # Optional per-trade notional history for volume-aware burst analytics (diagnostic; not used in current scoring)
        self._trade_notionals: deque = deque(maxlen=self.DEFAULT_HISTORY_MAXLEN)
        ...
        logger.info(f"  VPIN bucket (notional): {self.volume_bucket_units:.2f}")
    ...
    @staticmethod
    def _resolve_default_notional_bucket(symbol: str) -> float:
        """
        Resolve default VPIN notional bucket size (USD) from symbol.
        BTC/XBT -> 100,000; ETH -> 50,000; SOL -> 20,000; default -> 50,000.
        """
        u = symbol.upper()
        if u.startswith('BTC') or u.startswith('XBT'):
            return 100_000.0
        if u.startswith('ETH'):
            return 50_000.0
        if u.startswith('SOL'):
    ...
    def update_trade(self, size: float, price: float, side: int, timestamp_ns: int) -> None:
        ...
        Args:
            ...
            price: Trade price
            side: +1 for buy, -1 for sell (0 is ignored, directional)
        ...
        # Validate inputs and normalize side
        if not (np.isfinite(size) and np.isfinite(price)):
            return
        ...
        # NOTE: _trade_sizes stores RAW UNITS for burst-only analytics; VPIN uses USD notional
        trade_notional = size * price
        self._trade_notionals.append(trade_notional)
        remaining = trade_notional
        tol = max(self.volume_bucket_units * 1e-9, 1e-6)
        while remaining > 0.0:
            capacity = self.volume_bucket_units - self._current_volume
            if capacity <= tol:
                # Bucket is effectively full; finalize and continue
                self._finalize_bucket(timestamp_ns)
                continue

            take = remaining if remaining <= capacity else capacity

            # Count this trade's contribution to this bucket
            # 'take' is the USD-notional slice of trade_notional allocated to this bucket
            self._current_trade_count += 1

            self._current_volume += take
            if side > 0:
                self._current_buy_volume += take
            else:
                self._current_sell_volume += take

            remaining -= take

            # If bucket is now full (within epsilon), finalize
            if self._current_volume >= self.volume_bucket_units - tol:
                self._finalize_bucket(timestamp_ns)
    ...
    def _finalize_bucket(self, timestamp_ns: int) -> None:
        ...
        total_notional = self._current_volume
        buy_notional = self._current_buy_volume
        sell_notional = self._current_sell_volume

        # Notional imbalance (normalized to 0-1)
        imbalance = abs(buy_notional - sell_notional) / max(total_notional, EPS)

        # VPIN proxy (NOTIONAL): imbalance scaled by bucket fill, capped at 1
        vpin = min(1.0, imbalance * (total_notional / self.volume_bucket_units))

        # Store bucket data (in notional terms)
        self._bucket_volumes.append(total_notional)
        self._bucket_buy_volumes.append(buy_notional)
        self._bucket_sell_volumes.append(sell_notional)
        self._bucket_vpin.append(vpin)

        # Update activity history for anomaly detection (now notional)

        # Reset bucket (remainder is handled by slicing in update_trade)
        self._current_volume = 0.0
        self._current_buy_volume = 0.0
        self._current_sell_volume = 0.0
        self._current_trade_count = 0
        self._volume_history.append(total_notional)
    ...
    def update_candle(self, volume: float, high: float, low: float, close: float, timestamp_ns: int) -> None:
        ...
        self._volume_history.append(notional)

        # Create synthetic bucket for VPIN proxy using NOTIONAL proxy
        ...
        if len(self._candle_volumes) >= 2:
            ...
            recent_closes = list(self._candle_closes)[-self.lookback_buckets:]
            if recent_volumes and recent_closes and len(recent_volumes) == len(recent_closes):
                recent_notionals = [v * c for v, c in zip(recent_volumes, recent_closes)]
                avg_notional = np.mean(recent_notionals) if recent_notionals else 0.0
                if avg_notional > EPS:
                    # Proxy imbalance from range behavior - neutral directional assumption
                    vpin = min(1.0, notional / avg_notional)
                    self._bucket_vpin.append(vpin)
                    self._bucket_volumes.append(notional)
                    # No directional info in proxy mode
                    self._bucket_buy_volumes.append(notional / 2.0)
                    self._bucket_sell_volumes.append(notional / 2.0)
    ...
    def _calculate_volume_anomaly_score(self) -> float:
        """
        Calculate notional activity anomaly score (uses notional history).
        ...
        # Handle cases where z_score <= -2 to prevent NaN/inf
        if z_score <= -2:
            anomaly = 0.0
        else:
    ...
    def serialize_state(self) -> bytes:
        ...
        Schema version 4 (per-channel monotonic timestamps + full history continuity + buy/sell bucket splits):
        - version: uint8 (4)
        - symbol_len: uint8
        ...
        - current_volume (notional, in USD): float64
        - current_buy_volume (notional, in USD): float64
        - current_sell_volume (notional, in USD): float64
        - current_trade_count: uint32 (count of trades)
        ...
        - bucket_buy_volumes_count: uint16
        - bucket_buy_volumes_values (notional): float64[] (max 100)
        - bucket_sell_volumes_count: uint16
        - bucket_sell_volumes_values (notional): float64[] (max 100)
        - bucket_volumes_values (notional): float64[] (max 100)
        - volume_history_count: uint16
        - volume_history_values (notional): float64[] (max 100)
        ...
        bucket_buy_volumes_list = list(self._bucket_buy_volumes)[-100:]
        bucket_sell_volumes_list = list(self._bucket_sell_volumes)[-100:]
        ...
        # Pack bucket_buy_volumes array
        buy_count = len(bucket_buy_volumes_list)
        buy_fmt = f'>{buy_count}d' if buy_count > 0 else '>'
        buy_bytes = struct.pack('>H', buy_count) + (struct.pack(buy_fmt, *bucket_buy_volumes_list) if buy_count > 0 else b'')

        # Pack bucket_sell_volumes array
        sell_count = len(bucket_sell_volumes_list)
        sell_fmt = f'>{sell_count}d' if sell_count > 0 else '>'
        sell_bytes = struct.pack('>H', sell_count) + (struct.pack(sell_fmt, *bucket_sell_volumes_list) if sell_count > 0 else b'')
        ...
        return (header_bytes + venue_header + venue_bytes +
                vpin_bytes + vol_bytes + buy_bytes + sell_bytes +
    ...
    def deserialize_state(self, data: bytes) -> None:
        ...
        # bucket_buy_volumes
        buy_count = struct.unpack_from('>H', data, pos)[0]
        pos += 2
        if buy_count > 0:
            buy_fmt = f'>{buy_count}d'
            buy_values = struct.unpack_from(buy_fmt, data, pos)
            self._bucket_buy_volumes.clear()
            self._bucket_buy_volumes.extend(buy_values)
            pos += buy_count * 8

        # bucket_sell_volumes
        sell_count = struct.unpack_from('>H', data, pos)[0]
        pos += 2
        if sell_count > 0:
            sell_fmt = f'>{sell_count}d'
            sell_values = struct.unpack_from(sell_fmt, data, pos)
            self._bucket_sell_volumes.clear()
            self._bucket_sell_volumes.extend(sell_values)
            pos += sell_count * 8
